...
window = tk.Tk()
window.title("2025 ADP VORP KMeans Clustering")
...
ttk.Label(content_frame, text="X Axis:").grid(row=1, column=0, sticky='ew')
x_dropdown = ttk.Combobox(content_frame, textvariable=x_var, values=[axis_labels[a] for a in axis_options])
x_dropdown.grid(row=1, column=1, sticky='nsew')

y_var = tk.StringVar(value='VORP')
ttk.Label(content_frame, text="Y Axis:").grid(row=2, column=0, sticky='ew')
y_dropdown = ttk.Combobox(content_frame, textvariable=y_var, values=[axis_labels[a] for a in axis_options])
y_dropdown.grid(row=2, column=1, sticky='nsew')

# Number of clusters
ttk.Label(content_frame, text="Clusters:").grid(row=1, column=2, sticky='ew')
cluster_var = tk.IntVar(value=3)
cluster_spin = ttk.Spinbox(content_frame, from_=2, to=10, textvariable=cluster_var, width=5)
cluster_spin.grid(row=1, column=3, sticky='nsew')

# Player search and selection
player_search_var = tk.StringVar()
ttk.Label(content_frame, text="Search Player:").grid(row=3, column=0, sticky='ew')
player_search_entry = ttk.Entry(content_frame, textvariable=player_search_var)
player_search_entry.grid(row=3, column=1, sticky='nsew')

ttk.Label(content_frame, text="Player(s):").grid(row=4, column=0, sticky='ew')
player_listbox = tk.Listbox(content_frame, selectmode='multiple', exportselection=0, height=5)
player_listbox.grid(row=4, column=1, sticky='nsew', pady=2)

highlight_search_var = tk.StringVar()
ttk.Label(content_frame, text="Search Highlight:").grid(row=3, column=2, sticky='ew')
highlight_search_entry = ttk.Entry(content_frame, textvariable=highlight_search_var)
highlight_search_entry.grid(row=3, column=3, sticky='nsew')

ttk.Label(content_frame, text="Highlight Player(s):").grid(row=4, column=2, sticky='ew')
highlight_player_listbox = tk.Listbox(content_frame, selectmode='multiple', exportselection=0, height=5)
highlight_player_listbox.grid(row=4, column=3, sticky='nsew', pady=2)

ttk.Button(content_frame, text="Cluster", command=lambda: plot_clusters()).grid(row=5, column=0, columnspan=4, sticky='nsew')
...
def update_player_listbox(*args):
    ...
    filtered = filtered[filtered['AVG'] <= 300]
...
def plot_clusters():
    ...
    try:
        ...
        if filtered.empty:
            ...
            ax.set_title(f'KMeans Clustering: {axis_labels[y]} vs {axis_labels[x]}')
            ax.legend()
            fig.tight_layout()
            ...
            def on_add(sel):
                ...
                pos = filtered.iloc[idx]['POS']
                team = filtered.iloc[idx]['Team']
                ...
                text = (
                    f"{player} ({pos}, {team})\n"
                    f"Cluster: {cluster+1}\n"
                    f"{axis_labels[x]}: {x_val}\n"
                    f"{axis_labels[y]}: {y_val}"
                )
        ...
        plot_canvas.draw()
        plot_canvas.get_tk_widget().grid(row=8, column=0, columnspan=4, sticky='nsew')

        if not filtered.empty:
            first_player = filtered.iloc[0]['Player']
            first_pos = filtered.iloc[0]['POS']
            first_team = filtered.iloc[0]['Team']
            first_vorp = filtered.iloc[0]['VORP']
            first_avg = filtered.iloc[0]['AVG']
            first_vorp_vs_adp = df.iloc[0]['VORPvsADP_clean']
            selected_info_var.set(f"First Player: {first_player} ({first_pos}, {first_team}) - VORP: {first_vorp}, ADP AVG: {first_avg}, VORP vs ADP: {first_vorp_vs_adp}")
